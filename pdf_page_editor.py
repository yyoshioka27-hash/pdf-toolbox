...
def _reset_editor(source_id: str, source: PdfSource, pages: list[PageRef]) -> None:
    ...
    st.session_state.processed_drop_ids = set()
...
def _ensure_state() -> None:
    ...
    st.session_state.setdefault("processed_drop_ids", set())
...
def _delete_page(index: int) -> None:
    pages = st.session_state.pages
    if 0 <= index < len(pages):
        deleted_page = pages.pop(index)
        st.session_state.pages = pages
        _sync_order_text()
        st.toast(f"ページ {index + 1}（{deleted_page.source_name}）を削除しました。")


def _insert_pages_at(position: int, uploaded_files) -> None:
    ...
    added_count = 0
    insert_at = max(0, min(position, len(st.session_state.pages)))
    for uploaded_file in uploaded_files:
        file_bytes = uploaded_file.getvalue()
        drop_id = _file_digest(file_bytes, uploaded_file.name)
        if drop_id in st.session_state.processed_drop_ids:
            continue

        try:
            source_id, source, pages = _read_pdf(uploaded_file)
        except ValueError as exc:
            st.error(f"{uploaded_file.name}: {exc}")
            continue

        st.session_state.sources[source_id] = source
        st.session_state.pages[insert_at:insert_at] = pages
        st.session_state.processed_drop_ids.add(drop_id)
        insert_at += len(pages)
        added_count += len(pages)

    if added_count:
        _sync_order_text()
        st.toast(f"{added_count}ページを追加しました。")
        st.rerun()


def _render_drop_zone(position: int, label: str) -> None:
    uploaded_files = st.file_uploader(
        label,
        type=["pdf"],
        accept_multiple_files=True,
        key=f"drop_zone_{position}",
    )
    _insert_pages_at(position, uploaded_files)
...
def _render_page_list() -> None:
    ...
    st.caption(
        "サムネイルは表示用の低解像度画像です。各ページの Delete（削除）ボタンで削除でき、"
        "ページ間の点線枠にPDFをドロップすると、その位置にページを追加できます。"
    )

    with st.container(border=True):
        st.markdown("**ここにPDFをドロップすると先頭に追加**")
        _render_drop_zone(0, "先頭に追加するPDF")

    for index, page in enumerate(pages):
        with st.container(border=True):
            cols = st.columns([1.3, 2.4, 0.9, 0.9, 1.0])
            ...
            with cols[3]:
                if st.button("下へ", key=f"down_{index}", disabled=index == len(pages) - 1):
                    ...
                    st.rerun()
            with cols[4]:
                if st.button("Delete（削除）", key=f"delete_{index}", type="secondary"):
                    _delete_page(index)
                    st.rerun()

        with st.container(border=True):
            st.markdown(f"**ここにPDFをドロップするとページ {index + 1} の後ろに追加**")
            _render_drop_zone(index + 1, f"ページ {index + 1} の後ろに追加するPDF")
...
def main() -> None:
    ...
    st.header("ページ削除・PDF追加")
    st.write(
        "各ページの Delete（削除）ボタンでそのページを直接削除できます。"
        "ページとページの間にあるドロップ枠へPDFを入れると、その位置に追加されます。"
    )
